...
class CRM_Activity_Form_Activity extends CRM_Contact_Form_Task {
  ...
  public function preProcess() {
      ...
      foreach ($customGroupCount as $groupID => $groupCount) {
        if ($groupCount > 1) {
          ...
          for ($i = 0; $i <= $groupCount; $i++) {
            CRM_Custom_Form_CustomData::preProcess($this, NULL, $this->_activityTypeId, $i, 'Activity', $this->_activityId);
            CRM_Activity_Form_Edit_CustomData::buildQuickForm($this);
          }
        }
      }
    ...
    if ($this->_activityTypeId || isset($paramSubType)) {
